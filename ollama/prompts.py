...
Note:Don't be too loose in your evaluation Nor too strict, give accurate scoring If the outcome is correct, reasonable, and demonstrates clear logical thinking, give it a score of 5, even if the explanation is concise rather than extensive.
...
There are no situations like Ties or neither is a winner case always judgement_a or judgement_b
Conclude with a clear statement of which judgment is better than the other even slightly so using the JSON format with the 3 fields :
"{
    judgement_a : [judgement_a]
    judgement_b : [judgement_b]
    winner : [A winner of the both judgements "judgement_a" | "judgement_b", should always be only either judgement_a or judgement_b]
}"
"""

IMPROVE_PROMPT= """Given:

A response generated by the AI for that query in a predefined structure or format.
A judgement on that response provided by a judge.
Task: Analyze the judgement and use it to make necessary adjustments to the original response. The goal is to update the response based on the feedback while keeping the structure and data types intact. Scores, ratings, or any specific metrics present in the response should be carefully reviewed and adjusted according to the judgement, but they should not be drastically altered unless required by the feedback.

Process:

Review the judgement and identify areas for improvement.
Adjust the scores, ratings, or any other metrics in the original response if they are deemed inaccurate based on the judgement.
Make necessary modifications or additions to the response without drastically changing its content or format.
Ensure that the updated response is clear, concise, and properly addresses the feedback in the judgement.
Return Format:
The output should be returned in the same structure and format as the original response, only improved. Example:
Return only the final improved response in a jsom format given below :

{
    "improved_response": {
        // The improved response with necessary modifications made in accordance with the judgement.
    }
}
"""
